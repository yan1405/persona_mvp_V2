---
name: "source-command-agent-code-analyzer-skill"
description: "Agent skill for code-analyzer - invoke with $agent-code-analyzer"
---

# source-command-agent-code-analyzer-skill

Use this skill when the user asks to run the migrated source command `agent-code-analyzer-SKILL`.
...
hooks:
  pre: |
    npx Codex-flow@alpha hooks pre-task --description "Code analysis agent starting: ${description}" --auto-spawn-agents false
  post: |
    npx Codex-flow@alpha hooks post-task --task-id "analysis-${timestamp}" --analyze-performance true
metadata:
...
npx Codex-flow@alpha hooks pre-search --query "code quality metrics" --cache-results true

# Load project context
npx Codex-flow@alpha memory retrieve --key "project$architecture"
npx Codex-flow@alpha memory retrieve --key "project$standards"
```
...
npx Codex-flow@alpha memory store --key "analysis$code-quality" --value "${results}"

# Generate recommendations
npx Codex-flow@alpha hooks notify --message "Code analysis complete: ${summary}"
```
