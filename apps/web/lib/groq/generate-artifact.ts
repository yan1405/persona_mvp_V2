...
export async function generateArtifactSelection(input: {
  ...
  const messages = [
    ...
    { role: "user" as const, content: JSON.stringify({ tipo: input.type, objetivo: input.objective, contexto: input.context, modo: input.mode, blocos_solicitados: expectedBlocks, conteudo_atual: input.currentBody?.slice(0, 4_000) ?? null, evidencias: input.evidences }) },
  ];
  const request = () => groq.chat.completions.create({
    model, temperature: 0.1, max_completion_tokens: 2_400,
  ...
  try {
    ...
    try { completion = await request(); }
      ...
      if (!(error instanceof APIError) || error.status !== 400 || !error.message.includes("json_validate_failed")) throw error;
